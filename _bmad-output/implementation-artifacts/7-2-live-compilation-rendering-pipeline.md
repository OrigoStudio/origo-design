...
- [ ] Task 2: Wire Up Debounced Compilation in the Preview Service (AC: 1, 4, 5)
  ...
  - [ ] Implement debounce: accept editor content change events, debounce 400ms (balancing responsiveness vs load), then call `workerProxy.compile(content)`
  ...
  - [ ] Handle worker errors (terminate/restart strategy on unrecoverable crash)
...
- [ ] Task 3: Integrate with Preview Pane Component (AC: 2, 3)
  ...
  - [ ] Wrap the rendered preview output in a sandboxed `<iframe sandbox="allow-scripts allow-same-origin">` whose content is served from `src 'self'` origin
  - [ ] On `compiledAst` change (via `effect()`), post the resolved AST into the iframe via structured clone (CSP-safe `postMessage`)
...
- [ ] Task 4: Error Display Component (AC: 4)
  ...
  - [ ] Show a distinct "Syntax Error" vs "Semantic Error" badge per `CompilerError.type`
...
- **CSP / Worker instantiation:** Worker script MUST be a physical `.js` file fetched from `src 'self'`. Use `new Worker(new URL('./compiler.worker.ts', import.meta.url), { type: 'module' })` so Vite/Rollup emits a bundled file at build time. **NEVER use `URL.createObjectURL(new Blob([...]))` or inline worker blobs** — these violate `worker-src 'self'` CSP.
- **Comlink for RPC (AD-9 rule #2):** Use `comlink` for typed RPC between main thread and worker. Do NOT hand-roll `postMessage/onmessage` for the compiler API — it's error-prone and untyped. Wrap with `comlink.wrap<CompilerWorker>()` on the main side and `comlink.expose(new CompilerWorker())` on the worker side.
- **Preview iframe CSP (AC-3):** The preview `<iframe>` MUST have `sandbox="allow-scripts allow-same-origin"` attribute. AST data enters the iframe via `contentWindow.postMessage(ast, window.location.origin)`. The iframe content listens with `addEventListener('message', ...)` and verifies `event.origin`. **Do NOT set `allow-same-origin` alone without `allow-scripts` or the renderer won't function.**
...
- **AST transfer cost (for Story 7.3 continuity):** Use `structuredClone` (or `postMessage` structured clone) to transfer the AST from worker to main thread. If AST payloads exceed ~500KB, investigate `Transferable` ArrayBuffer encoding (flagged in ADR). Document the baseline transfer time in dev notes after implementation.
...
- `comlink` — already referenced in ADR; add to `packages/playground/package.json` if not present from 7.1 setup
